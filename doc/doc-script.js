...
function makeTree(treeData, root, filename) {
  let treeNode = document.getElementById("tree");
  let treeHandle = document.getElementById("sidebar-toggle");
...
function nodeClicked(e) {
  ...
  // Find the path and toggle the state, saving the state in the localStorage letiable
  let path = t.getAttribute("rel");
 ...
 */
function wireUpTabs() {
  let tabEl = document.getElementById("sidebar_switch");
  let children = tabEl.childNodes;
...
function switchTab(tab) {
  ...
  for (let i = 0, l = children.length; i < l; i += 1) {
    ...
    let t = children[i].className.replace(/\s.*$/, "");
    if (t === tab) {
